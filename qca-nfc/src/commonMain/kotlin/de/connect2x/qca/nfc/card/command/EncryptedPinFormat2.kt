...
internal class EncryptedPinFormat2(pin: String) {
    val bytes: ByteArray
        get() = field.copyOf()

    init {
        val intPin = pin.map { it.digitToInt() }
        ...
        intPin.forEach { digit ->
            require(digit in MIN_DIGIT..MAX_DIGIT) {
                "PIN digit value is out of range of a decimal digit: $digit"
            }
        }
